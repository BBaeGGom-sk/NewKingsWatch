...
<div class="collapse navbar-collapse" id="ftco-nav">
	...
	<div class="conP mContain">
        <ul class="bbList watch watch3Layout">
	        	...
	        	<div style="display: inline-block; float: right; margin-bottom: auto;">
					<a href="/goods/goodsSearchNoSelling" class="href_tag"><span class="spanAlign">NEW</span></a>
					<a href="/goods/rowPriceSearchNoSelling"	class="href_tag"><span class="spanAlign">ROW PRICE</span></a>
					<a href="/goods/highPriceSearchNoSelling"	class="href_tag"><span class="spanAlign">HIGH PRICE</span></a>
		        </div>
			<hr>
			<c:if test="${highPriceSearchNoSelling ne null}">
				<div style="clear: both; margin:auto; padding-left: 180px;">
					<img src="../../resources/img/noitems.png">
				</div>
			</c:if>
		<div style="clear: both;">
        <c:forEach items="${highPriceSearchNoSelling}" var="list"  varStatus="status">
            <li>
	 ...
	 <div class="row text-center">
		<ul class="pagination">
			<c:if test="${pm.cri.page>1}">
				<li><a href="/goods/highPriceSearchNoSelling${pm.makeSearch(pm.cri.page-1)}">&laquo;</a></li>
			</c:if>
				<c:forEach var="idx" begin="${pm.beginPageNum}"
					end="${pm.stopPageNum}">
					<li class="${pm.cri.page==idx?'active':''}"><a
						href="/goods/highPriceSearchNoSelling${pm.makeSearch(idx)}">${idx}</a></li>
				</c:forEach>
				<c:if test="${pm.cri.page<pm.totalPage}">
					<li><a href="/goods/highPriceSearchNoSelling${pm.makeSearch(pm.cri.page+1)}">&raquo;</a></li>
				</c:if>
			</ul>
		</div>
	...
	<script type="text/javascript">
		...
		// 반복문사용
		// # : 구분자
		<c:forEach items="${highPriceSearchNoSelling}" var="pic">
			arr=arr+"#"+"${pic.g_id}";			
		</c:forEach>
