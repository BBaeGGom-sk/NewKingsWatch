...
<body>
	...
	<section id="content">
	  	...
	  	<label for="rowPriceGoodsWoman">리스트입니다.</label>
		<div class="row">
	 ...
	 <hr>
	  <div id="container_box">
		 ...
		 <ul>
		 	<c:forEach items="${rowPriceGoodsWoman}" var="list"  varStatus="status">
			 	<li>
	 ...
	 <div class="row text-center">
		<ul class="pagination">
			<c:if test="${pm.cri.page>1}">
				<li><a href="/goods/rowPriceGoodsWoman${pm.makeCategory(pm.cri.page-1)}">&laquo;</a></li>
			</c:if>
				<c:forEach var="idx" begin="${pm.beginPageNum}"
					end="${pm.stopPageNum}">
					<li class="${pm.cri.page==idx?'active':''}"><a
						href="/goods/rowPriceGoodsWoman${pm.makeCategory(idx)}">${idx}</a></li>
				</c:forEach>
				<c:if test="${pm.cri.page<pm.totalPage}">
					<li><a href="/goods/rowPriceGoodsWoman${pm.makeCategory(pm.cri.page+1)}">&raquo;</a></li>
				</c:if>
			</ul>
		</div>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			// 반복문사용
			// # : 구분자
			<c:forEach items="${rowPriceGoodsWoman}" var="pic">
				arr=arr+"#"+"${pic.g_id}";			
			</c:forEach>
