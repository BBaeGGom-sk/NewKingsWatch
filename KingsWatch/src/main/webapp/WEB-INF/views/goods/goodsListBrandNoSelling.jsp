...
<div class="collapse navbar-collapse" id="ftco-nav">
	...
	<div class="conP mContain">
        <ul class="bbList watch watch3Layout">
	        	...
	        	<div style="display: inline-block; float: right; margin-bottom: auto;">
					<a href="/goods/goodsListBrandNoSelling?g_brand=로즈몽"><span class="spanAlign">NEW</span></a>
					<a href="/goods/goodsListBrandNoSelling?g_brand=세이코"><span class="spanAlign">ROW PRICE</span></a>
					<a href="/goods/goodsListBrandNoSelling?g_brand=아르마니"><span class="spanAlign">HIGH PRICE</span></a>
		        </div>
				<hr>
			<c:if test="${NoSelling ne null}">
				<div style="clear: both; margin:auto; padding-left: 180px;">
					<img src="../../resources/img/noitems.png">
				</div>
			</c:if>
		<div style="clear: both;">
        <c:forEach items="${NoSelling}" var="list"  varStatus="status">
            <li>
	 ...
	 <div class="row text-center">
		<ul class="pagination">
			<c:if test="${pm.cri.page>1}">
				<li><a href="/goods/goodsListBrandNoSelling${pm.makeBrand(pm.cri.page-1)}">&laquo;</a></li>
			</c:if>
				<c:forEach var="idx" begin="${pm.beginPageNum}"
					end="${pm.stopPageNum}">
					<li class="${pm.cri.page==idx?'active':''}"><a
						href="/goods/goodsListBrandNoSelling${pm.makeBrand(idx)}">${idx}</a></li>
				</c:forEach>
				<c:if test="${pm.cri.page<pm.totalPage}">
					<li><a href="/goods/goodsNoSelling${pm.makeBrand(pm.cri.page+1)}">&raquo;</a></li>
				</c:if>
			</ul>
		</div>
	...
	<script type="text/javascript">
		...
		// 반복문사용
		// # : 구분자
		<c:forEach items="${NoSelling}" var="pic">
			arr=arr+"#"+"${pic.g_id}";			
		</c:forEach>
