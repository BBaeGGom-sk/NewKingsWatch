...
<div class="collapse navbar-collapse" id="ftco-nav">
	...
	<div class="conP mContain">
        <ul class="bbList watch watch3Layout">
			...
			<hr>
			<c:if test="${searchNoSellGoodsList ne null}">
	 ...
	 <div class="row text-center">
		<ul class="pagination">
			<c:if test="${pm.cri.page>1}">
				<li><a href="/goods/goodsSearchNoSelling${pm.makeSearch(pm.cri.page-1)}">&laquo;</a></li>
			</c:if>
				<c:forEach var="idx" begin="${pm.beginPageNum}"
					end="${pm.stopPageNum}">
					<li class="${pm.cri.page==idx?'active':''}"><a
						href="/goods/goodsSearchNoSelling${pm.makeSearch(idx)}">${idx}</a></li>
				</c:forEach>
				<c:if test="${pm.cri.page<pm.totalPage}">
					<li><a href="/goods/goodsSearchNoSelling${pm.makeSearch(pm.cri.page+1)}">&raquo;</a></li>
				</c:if>
			</ul>
		</div>
