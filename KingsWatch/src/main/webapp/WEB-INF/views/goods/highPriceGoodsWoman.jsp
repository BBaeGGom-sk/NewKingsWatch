...
<body>
	...
	<section id="content">
	  	...
	  	<label for="highPriceGoodsWoman">리스트입니다.</label>
		<div class="row">
	 ...
	 <hr>
	  <div id="container_box">
	  	 <div>
		 	<a href="/goods/goodsListWoman" class="href_tag">최신순</a>
		 	<a href="/goods/rowPriceGoodsWoman"	class="href_tag">낮은가격순</a>
		 	<a href="/goods/highPriceGoodsWoman"	class="href_tag">높은가격순</a>
		 </div>
		 <ul>
		 	<c:forEach items="${highPriceGoodsWoman}" var="list"  varStatus="status">
			 	<li>
	 ...
	 <div class="row text-center">
		<ul class="pagination">
			<c:if test="${pm.cri.page>1}">
				<li><a href="/goods/highPriceGoodsWoman${pm.makeCategory(pm.cri.page-1)}">&laquo;</a></li>
			</c:if>
				<c:forEach var="idx" begin="${pm.beginPageNum}"
					end="${pm.stopPageNum}">
					<li class="${pm.cri.page==idx?'active':''}"><a
						href="/goods/highPriceGoodsWoman${pm.makeCategory(idx)}">${idx}</a></li>
				</c:forEach>
				<c:if test="${pm.cri.page<pm.totalPage}">
					<li><a href="/goods/highPriceGoodsWoman${pm.makeCategory(pm.cri.page+1)}">&raquo;</a></li>
				</c:if>
			</ul>
		</div>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			// 반복문사용
			// # : 구분자
			<c:forEach items="${highPriceGoodsWoman}" var="pic">
				arr=arr+"#"+"${pic.g_id}";			
			</c:forEach>
