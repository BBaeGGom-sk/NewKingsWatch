...
<body>
	...
	<section id="content">
	  	...
	  	<label for="rowPriceGoodsPublic">공용상품 리스트입니다.</label>
		<div class="row">
	 ...
	 <hr>
	  <div id="container_box">
		 ...
		 <ul>
		 	<c:forEach items="${rowPriceGoodsPublic}" var="list"  varStatus="status">
			 	<li>
			...
			<c:if test="${pm.cri.page>1}">
				...
				<c:forEach var="idx" begin="${pm.beginPageNum}"
					...
					<li class="${pm.cri.page==idx?'active':''}"><a
						href="/goods/rowPriceGoodsPublic${pm.makeCategory(idx)}">${idx}</a></li>
				</c:forEach>
				<c:if test="${pm.cri.page<pm.totalPage}">
					<li><a href="/goods/rowPriceGoodsPublic${pm.makeCategory(pm.cri.page+1)}">&raquo;</a></li>
				</c:if>
			</ul>
		</div>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			// 반복문사용
			// # : 구분자
			<c:forEach items="${rowPriceGoodsPublic}" var="pic">
				arr=arr+"#"+"${pic.g_id}";			
			</c:forEach>
