...
<body>
	...
	<div class="m_container">
		<div class="row">
			...
			<nav id="nav">
				...
				<ul class="goodsMenu">
					<li class="menu0"><a href="/goods/goodsList">메인으로</a></li>
					<li class="menu1"><a href="/goods/goodsListBrand?g_brand=로즈몽">로즈몽</a>
						<ul class="row submenu">
							...
							<li><a
								href="/goods/goodsListPublic?g_category=0&g_brand=로즈몽">공용</a></li>
						</ul></li>
					<li class="menu2"><a href="/goods/goodsListBrand?g_brand=세이코">세이코</a>
						<ul class="row submenu">
							...
							<li><a
								href="/goods/goodsListPublic?g_category=0&g_brand=세이코">공용</a></li>
						</ul></li>
					<li class="menu3"><a href="/goods/goodsListBrand?g_brand=아르마니">아르마니</a>
						<ul class="row submenu">
							<li><a
								href="/goods/goodsListWoman?g_category=1&g_brand=아르마니">여성</a></li>
							<li><a href="/goods/goodsListMan?g_category=2&g_brand=아르마니">남성</a></li>
							<li><a
								href="/goods/goodsListPublic?g_category=0&g_brand=아르마니">공용</a></li>
						</ul></li>
			...
			<div class="conP mContain">
				<ul class="bbList watch watch3Layout">
					...
					<div
						style="display: inline-block; float: right; margin-bottom: auto;">
						<a href="/goods/goodsSearch" class="href_tag"><span
							class="spanAlign">NEW</span></a> <a href="/goods/rowPriceGoodsSearch"
							class="href_tag"><span class="spanAlign">ROW PRICE</span></a> <a
							href="/goods/highPriceGoodsSearch" class="href_tag"><span
							class="spanAlign">HIGH PRICE</span></a>
					</div>
					<hr>
					
					<c:if test="${highPriceGoodsSearch ne null}">
					...
					<div style="clear: both;">
						<c:forEach items="${highPriceGoodsSearch}" var="list"
							varStatus="status">
							<li><a href="/goods/goodsRead?g_id=${list.g_id}">
									<div id="${status.index}" class="uploadedList"></div>
									<div class="textWrap">
										<div class="txtBox">
											<p class="pdName">${list.g_name}</p>
											<p class="pdDetail">
												&#8361;
												<fmt:formatNumber value="${list.g_price}" type="number" />
											</p>
										</div>
									</div>
							</a></li>
		...
		<div class="row text-center">
			<ul class="pagination">
				<c:if test="${pm.cri.page>1}">
					<li><a
						href="/goods/highPriceGoodsSearch${pm.makeSearch(pm.cri.page-1)}">&laquo;</a></li>
				</c:if>
				<c:forEach var="idx" begin="${pm.beginPageNum}"
					end="${pm.stopPageNum}">
					<li class="${pm.cri.page==idx?'active':''}"><a
						href="/goods/highPriceGoodsSearch${pm.makeSearch(idx)}">${idx}</a></li>
				</c:forEach>
				<c:if test="${pm.cri.page<pm.totalPage}">
					<li><a
						href="/goods/highPriceGoodsSearch${pm.makeSearch(pm.cri.page+1)}">&raquo;</a></li>
				</c:if>
			</ul>
		</div>
	...
	<div class="form-group"
		...
		<form method="get" action="/goods/goodsSearch">
			...
			<div class="input-group col-xs-1 col-sm-1">
				<input class="form-control" id="keyword" name="keyword" size="20px"
					style="text-align: center; width: 200px;"> <span
	...
	<script type="text/javascript">
		...
		// 반복문사용
		// # : 구분자
		<c:forEach items="${highPriceGoodsSearch}" var="pic">
			arr=arr+"#"+"${pic.g_id}";			
		</c:forEach>
