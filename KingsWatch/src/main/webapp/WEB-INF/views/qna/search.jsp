				...
				<tbody>
					<c:forEach var="vo" items="${list}">
						<tr>
							<td>${vo.q_bno}</td>
							<td><a
								href="/qna/sread${pm.makeQuery(pm.cri.page)}&q_bno=${vo.q_bno}">
									<c:choose>
		...
		<div class="row text-center">
			<ul class="pagination">
				<c:if test="${pm.cri.page>1}">
					<li><a href="/qna/search${pm.makeQuery(pm.cri.page-1)}">&laquo;</a></li>
				</c:if>
				...
				<c:forEach var="idx" begin="${pm.beginPageNum}"
					...
					<li class="${pm.cri.page==idx?'active':''}"><a
						href="/qna/search${pm.makeQuery(idx)}">${idx}</a></li>
				</c:forEach>
				<c:if test="${pm.cri.page<pm.totalPage}">
					<li><a href="/qna/search${pm.makeQuery(pm.cri.page+1)}">&raquo;</a></li>
				</c:if>
			</ul>
